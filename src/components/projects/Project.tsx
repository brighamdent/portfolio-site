...
export const Project: React.FC<ProjectProps> = ({ projectData, index }) => {
  ...
  return (
    <motion.div
      ...
      className={`card flex flex-col lg:flex-row justify-around items-center w-[300px] md:w-[600px] lg:w-[900px] rounded-3xl lg:h-[360px] mt-6 p-5 ${projectData.layout == "left" ? "flex-row-reverse" : ""}`}
    >
      <div className="flex flex-col p-4 items-center lg:items-start lg:w-1/3 ">
        <div className="flex justify-around ">
          <h2 className="text-[24px] md:text-[32px]">{projectData.name}</h2>
          <img
        ...
        <p className="text-[16px] md:text-[20px] lg:text-left">{projectData.description}</p>
      </div>
